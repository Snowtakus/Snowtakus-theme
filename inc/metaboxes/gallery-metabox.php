 ...
 *
 */
function snowtakus_gallery_meta_box_display() {
	global $post;

	$featured_image = get_post_meta($post->ID, 'gallery_featured_image_url_field', true);

	wp_nonce_field( 'snowtakus_gallery_meta_box_nonce', 'snowtakus_gallery_meta_box_nonce' );
	?>
	...
	wp_editor( htmlspecialchars_decode( $editor_text ), 'snowtakus_gallery_wysiwyg', $settings = array( 'textarea_name' => 'gallery-wysiwyg' ) );
}

/**
 *
 */
function snowtakus_gallery_meta_box_save($post_id) {
	if (!isset($_POST['snowtakus_gallery_meta_box_nonce'])
		|| !wp_verify_nonce($_POST['snowtakus_gallery_meta_box_nonce'], 'snowtakus_gallery_meta_box_nonce')) {
		return;
	}
...
add_action('save_post', 'snowtakus_gallery_meta_box_save');
